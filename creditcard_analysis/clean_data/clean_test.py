...
def download_csv(csv_url: str, save_dir: Path, cafile: str | None = None) -> Path:
    ...
    file_path = save_dir / f"raw_open_data{timestamp}.csv"
...
def clean_data(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if missing:
        print(f"缺失欄位：{missing} 目前欄位：{df.columns.to_list()}")
...
# Load to DB (MySQL)
# =========================
def insert_into_msyql(df: pd.DataFrame, table_name: str = "clean_data", chunk_size: int = 1000) -> None:
...
def run_etl(opts: ETLOptions) -> None:
    ...
    insert_into_msyql(df, opts.table_name)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--url", default = None, help = "下載CSV時的URL")
    p.add_argument("--file", default = None, help = "本機JSON / JSONL 檔案")
    p.add_argument("--out_dir", default = str(DATA_DIR), help = "下載檔案資料夾 (csv 模式用)")
